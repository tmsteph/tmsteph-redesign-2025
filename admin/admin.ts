(function () {
  ...
  type GunInstance = {
    ...
    _?: { opt?: { peers?: Record<string, { wire?: unknown }> } };
  };
  type GunStatic = {
    (options?: { peers?: readonly string[]; localStorage?: boolean }): GunInstance;
    SEA?: {
      encrypt?: (data: any, key: unknown) => Promise<unknown>;
      decrypt?: (data: any, key: unknown) => Promise<unknown>;
    };
    text?: { random?: (length?: number) => string };
  };

  const root = typeof window !== 'undefined' ? (window as Window & { Gun?: GunStatic }) : undefined;
  const Gun = root?.Gun;
  ...
  const authSection = getElement<HTMLElement>('auth-section');
  const adminPanel = getElement<HTMLElement>('admin-panel');
  ...
  const authToggleText = getElement<HTMLElement>('auth-toggle-text');
  const toggleAuthBtn = getElement<HTMLButtonElement>('toggle-auth');
  const restoreSessionBtn = getElement<HTMLButtonElement>('restore-session');
  const authMessage = getElement<HTMLElement>('auth-message');
  const panelMessage = getElement<HTMLElement>('panel-message');
  const aliasDisplay = getElement<HTMLElement>('alias-display');
  const logoutBtn = getElement<HTMLButtonElement>('logout-btn');

  const statusForm = getElement<HTMLFormElement>('status-form');
  const statusInput = getElement<HTMLInputElement>('status-input');
  const statusPreview = getElement<HTMLElement>('status-preview');

  const noteForm = getElement<HTMLFormElement>('note-form');
  const noteInput = getElement<HTMLTextAreaElement>('note-input');
  const notePreview = getElement<HTMLElement>('note-preview');

  const commandCentralForm = getElement<HTMLFormElement>('command-central-form');
  const commandCentralToggle = getElement<HTMLInputElement>('command-central-toggle');
  const commandCentralPreview = getElement<HTMLElement>('command-central-preview');

  const photoForm = getElement<HTMLFormElement>('photo-upload-form');
  const photoFileInput = getElement<HTMLInputElement>('photo-file');
  const photoCaptionInput = getElement<HTMLInputElement>('photo-caption');
  const photoList = getElement<HTMLElement>('photo-list');
  const photoMessage = getElement<HTMLElement>('photo-message');

  const aliasInput = getElement<HTMLInputElement>('alias');
  const passwordInput = getElement<HTMLInputElement>('password');
  const passwordToggle = getElement<HTMLButtonElement>('password-toggle');
  ...
  const attachPhotoLibraryListeners = () => {
    ...
    const attach = (
      node: { map?: () => { on?: (callback: (record: any, key: string) => void) => void } } | null,
      ...
      node.map().on(async (record, key) => {
        ...
        try {
          ...
          const decryptedRecord =
            decrypted && typeof decrypted === 'object' ? (decrypted as Record<string, any>) : null;
  ...
  const restoreSavedAccount = () => {
    ...
    const pair = user._?.sea as { pub?: string; epub?: string; priv?: string; epriv?: string } | undefined;
    ...
    user.create(alias, pair as any, (ack) => {
      if (ack?.err) {
  ...
  const updateAliasDisplay = (aliasValue?: string) => {
    if (!aliasDisplay) {
      return;
    }
  ...
  if (photoList) {
    photoList.addEventListener('click', (event) => {
      ...
      const deleteBtn = target?.closest('[data-photo-delete="true"]') as HTMLElement | null;
      if (!deleteBtn) {
        return;
      }
